...
namespace tcpdf;
use \tcpdf\font\Fonts;
use \tcpdf\font\FontData;
use \tcpdf\barcodes\Barcode;
use \tcpdf\barcodes\Barcode2D;
use \tcpdf\basic\Statics;
use \tcpdf\webbasedcolor\Colors;
//$icc = file_get_contents(dirname(__FILE__).'/common/sRGB.icc'); THIS IS A DEPENDENCY
...
class Tcpdf {
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->file_id = md5(Statics::getRandomSeed('TCPDF'.$orientation.$unit.$serformat.$encoding));
	...
	protected function setPageFormat($format, $orientation='P') {
		...
		if (is_string($format)) {
			...
			$pf = Statics::getPageSizeFromFormat($format);
			$this->fwPt = $pf[0];
			$this->fhPt = $pf[1];
		} else {
			// the boundaries of the physical medium on which the page shall be displayed or printed
			if (isset($format['MediaBox'])) {
				$this->pagedim = Statics::setPageBoxes($this->page, 'MediaBox', $format['MediaBox']['llx'], $format['MediaBox']['lly'], $format['MediaBox']['urx'], $format['MediaBox']['ury'], false, $this->k, $this->pagedim);
				...
				if (isset($format[0]) AND is_numeric($format[0]) AND isset($format[1]) AND is_numeric($format[1])) {
					...
					$pf = Statics::getPageSizeFromFormat($format['format']);
				}
				$this->fwPt = $pf[0];
				$this->fhPt = $pf[1];
				$this->pagedim = Statics::setPageBoxes($this->page, 'MediaBox', 0, 0, $this->fwPt, $this->fhPt, true, $this->k, $this->pagedim);
			}
			// the visible region of default user space
			if (isset($format['CropBox'])) {
				$this->pagedim = Statics::setPageBoxes($this->page, 'CropBox', $format['CropBox']['llx'], $format['CropBox']['lly'], $format['CropBox']['urx'], $format['CropBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the region to which the contents of the page shall be clipped when output in a production environment
			if (isset($format['BleedBox'])) {
				$this->pagedim = Statics::setPageBoxes($this->page, 'BleedBox', $format['BleedBox']['llx'], $format['BleedBox']['lly'], $format['BleedBox']['urx'], $format['BleedBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the intended dimensions of the finished page after trimming
			if (isset($format['TrimBox'])) {
				$this->pagedim = Statics::setPageBoxes($this->page, 'TrimBox', $format['TrimBox']['llx'], $format['TrimBox']['lly'], $format['TrimBox']['urx'], $format['TrimBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the page's meaningful content (including potential white space)
			if (isset($format['ArtBox'])) {
				$this->pagedim = Statics::setPageBoxes($this->page, 'ArtBox', $format['ArtBox']['llx'], $format['ArtBox']['lly'], $format['ArtBox']['urx'], $format['ArtBox']['ury'], false, $this->k, $this->pagedim);
			}
	...
	public function setPageOrientation($orientation, $autopagebreak='', $bottommargin='') {
		...
		if (!isset($this->pagedim[$this->page]['CropBox'])) {
			// the visible region of default user space
			$this->pagedim = Statics::setPageBoxes($this->page, 'CropBox', $this->pagedim[$this->page]['MediaBox']['llx'], $this->pagedim[$this->page]['MediaBox']['lly'], $this->pagedim[$this->page]['MediaBox']['urx'], $this->pagedim[$this->page]['MediaBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['BleedBox'])) {
			// the region to which the contents of the page shall be clipped when output in a production environment
			$this->pagedim = Statics::setPageBoxes($this->page, 'BleedBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['TrimBox'])) {
			// the intended dimensions of the finished page after trimming
			$this->pagedim = Statics::setPageBoxes($this->page, 'TrimBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['ArtBox'])) {
			// the page's meaningful content (including potential white space)
			$this->pagedim = Statics::setPageBoxes($this->page, 'ArtBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		...
		if ((abs($this->pagedim[$this->page]['MediaBox']['urx'] - $this->hPt) < $this->feps) AND (abs($this->pagedim[$this->page]['MediaBox']['ury'] - $this->wPt) < $this->feps)){
			...
			$this->pagedim = Statics::swapPageBoxCoordinates($this->page, $this->pagedim);
		}
		$this->w = ($this->wPt / $this->k);
		$this->h = ($this->hPt / $this->k);
		if (Statics::empty_string($autopagebreak)) {
		...
		if (Statics::empty_string($bottommargin)) {
	...
	public function SetDisplayMode($zoom, $layout='SinglePage', $mode='UseNone') {
		...
		$this->LayoutMode = Statics::getPageLayoutMode($layout);
		$this->PageMode = Statics::getPageMode($mode);
	}
	...
	protected function setTableHeader() {
		...
		if (!Statics::empty_string($this->thead) AND (!$this->inthead)) {
			// set margins
	...
	public function GetArrStringWidth($sa, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		...
		if (!Statics::empty_string($fontname)) {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (Statics::empty_string($family)) {
			if (!Statics::empty_string($this->FontFamily)) {
		...
		$fontdir = false;
		if (!Statics::empty_string($fontfile)) {
			$fontdir = dirname($fontfile);
			if (Statics::empty_string($fontdir) OR ($fontdir == '.')) {
				$fontdir = '';
			} else {
				$fontdir .= '/';
			}
		...
		if (Statics::empty_string($fontfile) OR (!@file_exists($fontfile))) {
		...
		// include font file
		if (!Statics::empty_string($fontfile) AND (@file_exists($fontfile))) {
			include($fontfile);
		} else {
		...
		if (!isset($file) OR Statics::empty_string($file)) {
			$file = '';
		}
		if (!isset($enc) OR Statics::empty_string($enc)) {
			$enc = '';
		}
		if (!isset($cidinfo) OR Statics::empty_string($cidinfo)) {
			$cidinfo = array('Registry'=>'Adobe', 'Ordering'=>'Identity', 'Supplement'=>0);
			$cidinfo['uni2cid'] = array();
		}
		if (!isset($ctg) OR Statics::empty_string($ctg)) {
			$ctg = '';
		}
		if (!isset($desc) OR Statics::empty_string($desc)) {
			$desc = array();
		}
		if (!isset($up) OR Statics::empty_string($up)) {
			$up = -100;
		}
		if (!isset($ut) OR Statics::empty_string($ut)) {
			$ut = 50;
		}
		if (!isset($cw) OR Statics::empty_string($cw)) {
			$cw = array();
		}
		if (!isset($dw) OR Statics::empty_string($dw)) {
			// set default width
	...
	public function isCharDefined($char, $font='', $style='') {
		...
		if (Statics::empty_string($font)) {
			if (Statics::empty_string($style)) {
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode) {
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!Statics::empty_string($opt['FS']))
				AND (@file_exists($opt['FS']) OR Statics::isValidURL($opt['FS']))
	...
	protected function _putEmbeddedFiles() {
		...
		foreach ($this->embeddedfiles as $filename => $filedata) {
			$data = Statics::fileGetContents($filedata['file']);
			if ($data !== FALSE) {
				$rawsize = strlen($data);
				if ($rawsize > 0) {
					// update name tree
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		$txt = Statics::removeSHY($txt, $this->isunicode);
		...
		if (Statics::empty_string($w) OR ($w <= 0)) {
			if ($this->rtl) {
				$w = $x - $this->lMargin;
			} else {
	...
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
		...
		$this->setCellMargins(0, 0, 0, 0);
		if (Statics::empty_string($this->lasth) OR $reseth) {
			// reset row height
			$this->resetLastH();
		}
		if (!Statics::empty_string($y)) {
		...
		// get border modes
		$border_start = Statics::getBorderMode($border, $position='start', $this->opencell);
		$border_end = Statics::getBorderMode($border, $position='end', $this->opencell);
		$border_middle = Statics::getBorderMode($border, $position='middle', $this->opencell);
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						...
						$nextstr = Statics::pregSplit('/'.$this->re_space['p'].'/', $this->re_space['m'], $this->stringTrim($strrest));
		...
		if ($file[0] === '@') {
			...
			if (($imsize = @getimagesize($file)) === FALSE) {
				if (in_array($file, $this->imagekeys)) {
					...
					$imgdata = Statics::fileGetContents($file);
				}
			}
		}
		if (!empty($imgdata)) {
			// copy image to cache
			$original_file = $file;
			$file = Statics::getObjFilename('img', $this->file_id);
			$fp = Statics::fopenLocal($file, 'w');
			if (!$fp) {
			...
			if (($info === false) AND function_exists($gdfunction)) {
				try {
					...
					if ($img !== false) {
						...
						if (($type == 'gif') OR ($type == 'png')) {
							$info = Images::_toPNG($img, Statics::getObjFilename('img', $this->file_id));
						} else {
							$info = Images::_toJPEG($img, $this->jpeg_quality, Statics::getObjFilename('img', $this->file_id));
						}
					}
				} catch(Exception $e) {
					$info = false;
				...
				try {
					...
					if ($type == 'svg') {
						if ($file[0] === '@') {
							...
							$svgimg = Statics::fileGetContents($file);
						}
						if ($svgimg !== FALSE) {
					...
					$tempname = Statics::getObjFilename('img', $this->file_id);
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			// remove filler space
			$byterange_string_len = strlen(Statics::$byterange_string);
			// define the ByteRange
			$byte_range = array();
			$byte_range[0] = 0;
			$byte_range[1] = strpos($pdfdoc, Statics::$byterange_string) + $byterange_string_len + 10;
			...
			$pdfdoc = str_replace(Statics::$byterange_string, $byterange, $pdfdoc);
			// write the document to a temporary folder
			$tempdoc = Statics::getObjFilename('doc', $this->file_id);
			$f = Statics::fopenLocal($tempdoc, 'wb');
			if (!$f) {
			...
			$tempsign = Statics::getObjFilename('sig', $this->file_id);
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					Statics::sendOutputData($this->getBuffer(), $this->bufferlen);
				} else {
					echo $this->getBuffer();
				}
				break;
			...
			case 'FD': {
				...
				if ($dest == 'FI') {
					...
					header('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT');
					header('Content-Disposition: inline; filename="'.basename($name).'"');
					Statics::sendOutputData(file_get_contents($name), filesize($name));
				} elseif ($dest == 'FD') {
					// send headers to browser
					if (ob_get_contents()) {
						$this->Error('Some data has already been output, can\'t send PDF file');
	...
	protected function getInternalPageNumberAliases($a= '') {
		...
		$u = '{'.$a.'}';
		$alias['u'][] = Statics::_escape($u);
		if ($this->isunicode) {
			$alias['u'][] = Statics::_escape(Fonts::UTF8ToLatin1($u, $this->isunicode, $this->CurrentFont));
			$alias['u'][] = Statics::_escape(Fonts::utf8StrRev($u, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = Statics::_escape(Fonts::UTF8ToLatin1($a, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = Statics::_escape(Fonts::utf8StrRev($a, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
		}
		$alias['a'][] = Statics::_escape($a);
		return $alias;
	}
	...
	protected function getAllInternalPageNumberAliases() {
		$basic_alias = array(Statics::$alias_tot_pages, Statics::$alias_num_page, Statics::$alias_group_tot_pages, Statics::$alias_group_num_page, Statics::$alias_right_shift);
		$pnalias = array();
	...
	protected function setPageBoxTypes($boxes) {
		...
		foreach ($boxes as $box) {
			if (in_array($box, Statics::$pageboxes)) {
				$this->page_boxes[] = $box;
			}
		}
	}
	...
	protected function _putpages() {
		...
		$ptpa = Statics::formatPageNumber(($this->starting_page_number + $num_pages - 1));
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$pnpa = Statics::formatPageNumber(($this->starting_page_number + $n - 1));
			...
			if (!empty($this->pagegroups)) {
				...
				list($temppage, $gdiff) = Statics::replacePageNumAliases($temppage, $replace, $gdiff);
			}
			// replace page numbers
			$replace = array();
			...
			list($temppage, $pdiff) = Statics::replacePageNumAliases($temppage, $replace, $pdiff);
						...
						case 'link': {
							if (is_string($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									$annots .= ' /Dest /'.Statics::encodeNameObject(substr($pl['txt'], 1));
								} elseif ($pl['txt'][0] == '%') {
									// embedded PDF file
	...
	protected function _putfonts() {
		...
		foreach ($this->fontkeys as $k) {
			...
			if ($type == 'core') {
				...
				if (!Statics::empty_string($font['file'])) {
	...
	protected function _puttruetypeunicode($font) {
		...
		if (isset($font['ctg']) AND (!Statics::empty_string($font['ctg']))) {
	...
	protected function _putinfo() {
		...
		if (!Statics::empty_string($this->title)) {
			// The document's title.
			$out .= ' /Title '.$this->_textstring($this->title, $oid);
		}
		if (!Statics::empty_string($this->author)) {
			// The name of the person who created the document.
			$out .= ' /Author '.$this->_textstring($this->author, $oid);
		}
		if (!Statics::empty_string($this->subject)) {
			// The subject of the document.
			$out .= ' /Subject '.$this->_textstring($this->subject, $oid);
		}
		if (!Statics::empty_string($this->keywords)) {
			// Keywords associated with the document.
			$out .= ' /Keywords '.$this->_textstring($this->keywords, $oid);
		}
		if (!Statics::empty_string($this->creator)) {
		...
		// default producer
		$out .= ' /Producer '.$this->_textstring(Statics::getTCPDFProducer(), $oid);
	 ...
	 * Be careful to add a well formattedcustom metadata section with namespaces, valid tags!
	...
	protected function _putXMP() {
		...
		$xmp .= "\t\t\t".'<dc:title>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">'.Statics::_escapeXML($this->title).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:title>'."\n";
		$xmp .= "\t\t\t".'<dc:creator>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Seq>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.Statics::_escapeXML($this->author).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Seq>'."\n";
		$xmp .= "\t\t\t".'</dc:creator>'."\n";
		$xmp .= "\t\t\t".'<dc:description>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">'.Statics::_escapeXML($this->subject).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:description>'."\n";
		$xmp .= "\t\t\t".'<dc:subject>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Bag>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.Statics::_escapeXML($this->keywords).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Bag>'."\n";
		$xmp .= "\t\t\t".'</dc:subject>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		// convert doc creation date format
		$dcdate = Statics::getFormattedDate($this->doc_creation_timestamp);
		...
		$doccreationdate = Statics::_escapeXML($doccreationdate);
		// convert doc modification date format
		$dmdate = Statics::getFormattedDate($this->doc_modification_timestamp);
		...
		$docmoddate = Statics::_escapeXML($docmoddate);
		...
		$xmp .= "\t\t\t".'<pdf:Keywords>'.Statics::_escapeXML($this->keywords).'</pdf:Keywords>'."\n";
		$xmp .= "\t\t\t".'<pdf:Producer>'.Statics::_escapeXML(Statics::getTCPDFProducer()).'</pdf:Producer>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
	...
	protected function _putcatalog() {
		...
		if ($this->pdfa_mode OR $this->force_srgb) {
			$iccobj = $this->_newobj();
			$icc = file_get_contents(dirname(__FILE__).'/common/sRGB.icc');
			$filter = '';
			if ($this->compress) {
				$filter = ' /Filter /FlateDecode';
				$icc = gzcompress($icc);
		...
		if (isset($this->LayoutMode) AND (!Statics::empty_string($this->LayoutMode))) {
			$out .= ' /PageLayout /'.$this->LayoutMode;
		}
		if (isset($this->PageMode) AND (!Statics::empty_string($this->PageMode))) {
	...
	protected function _putviewerpreferences() {
		...
		if (isset($vp['Duplex']) AND (!Statics::empty_string($vp['Duplex']))) {
	...
	protected function _beginpage($orientation='', $format='') {
		...
		$this->state = 2;
		if (Statics::empty_string($orientation)) {
			if (isset($this->CurOrientation)) {
		...
		if (Statics::empty_string($format)) {
	...
	public function setDocCreationTimestamp($time) {
		if (is_string($time)) {
			$time = Statics::getTimestamp($time);
		}
	...
	protected function _datestring($n=0, $timestamp=0) {
		...
		return $this->_datastring('D:'.Statics::getFormattedDate($timestamp), $n);
	}
	...
	protected function _objectkey($n) {
		...
		$objkey = substr(Statics::_md5_16($objkey), 0, (($this->encryptdata['Length'] / 8) + 5));
	...
	protected function _encrypt_data($n, $s) {
		...
		switch ($this->encryptdata['mode']) {
			case 0:   // RC4-40
			case 1: { // RC4-128
				$s = Statics::_RC4($this->_objectkey($n), $s, $this->last_enc_key, $this->last_enc_key_c);
				break;
			}
			case 2: { // AES-128
				$s = Statics::_AES($this->_objectkey($n), $s);
				break;
			}
			case 3: { // AES-256
				$s = Statics::_AES($this->encryptdata['key'], $s);
				break;
			}
		}
		return $s;
	...
	protected function _putencryption() {
		...
		if ($this->encryptdata['pubkey']) {
			...
			if ($this->encryptdata['V'] == 5) { // AES-256
				$out .= ' 5';
				$out .= ' /OE ('.Statics::_escape($this->encryptdata['OE']).')';
				$out .= ' /UE ('.Statics::_escape($this->encryptdata['UE']).')';
				$out .= ' /Perms ('.Statics::_escape($this->encryptdata['perms']).')';
			...
			$out .= ' /O ('.Statics::_escape($this->encryptdata['O']).')';
			$out .= ' /U ('.Statics::_escape($this->encryptdata['U']).')';
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { // RC4-40
			return Statics::_RC4($this->encryptdata['key'], Statics::$enc_padding, $this->last_enc_key, $this->last_enc_key_c);
		} elseif ($this->encryptdata['mode'] < 3) { // RC4-128, AES-128
			$tmp = Statics::_md5_16(Statics::$enc_padding.$this->encryptdata['fileid']);
			$enc = Statics::_RC4($this->encryptdata['key'], $tmp, $this->last_enc_key, $this->last_enc_key_c);
			$len = strlen($tmp);
			for ($i = 1; $i <= 19; ++$i) {
				$ek = '';
				for ($j = 0; $j < $len; ++$j) {
				...
				$enc = Statics::_RC4($ek, $enc, $this->last_enc_key, $this->last_enc_key_c);
			}
			$enc .= str_repeat("\x00", 16);
			return substr($enc, 0, 32);
		} elseif ($this->encryptdata['mode'] == 3) { // AES-256
			$seed = Statics::_md5_16(Statics::getRandomSeed());
	...
	protected function _UEvalue() {
		...
		return Statics::_AESnopad($hashkey, $this->encryptdata['key']);
	}
	...
	protected function _Ovalue() {
		if ($this->encryptdata['mode'] < 3) { // RC4-40, RC4-128, AES-128
			$tmp = Statics::_md5_16($this->encryptdata['owner_password']);
			if ($this->encryptdata['mode'] > 0) {
				for ($i = 0; $i < 50; ++$i) {
					$tmp = Statics::_md5_16($tmp);
				}
			}
			$owner_key = substr($tmp, 0, ($this->encryptdata['Length'] / 8));
			$enc = Statics::_RC4($owner_key, $this->encryptdata['user_password'], $this->last_enc_key, $this->last_enc_key_c);
			if ($this->encryptdata['mode'] > 0) {
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			if ($this->encryptdata['mode'] == 3) { // AES-256
				...
				$this->encryptdata['key'] = substr(hash('sha256', Statics::getRandomSeed(), true), 0, $keybytelen);
				// truncate passwords
				...
				$perms = Statics::getEncPermissionsString($this->encryptdata['protection']); // bytes 0-3
				...
				$this->encryptdata['perms'] = Statics::_AESnopad($this->encryptdata['key'], $perms);
			} else { // RC4-40, RC4-128, AES-128
				// Pad passwords
				$this->encryptdata['user_password'] = substr($this->encryptdata['user_password'].Statics::$enc_padding, 0, 32);
				$this->encryptdata['owner_password'] = substr($this->encryptdata['owner_password'].Statics::$enc_padding, 0, 32);
				// Compute O value
				$this->encryptdata['O'] = $this->_Ovalue();
				// get default permissions (reverse byte order)
				$permissions = Statics::getEncPermissionsString($this->encryptdata['protection']);
				// Compute encryption key
				$tmp = Statics::_md5_16($this->encryptdata['user_password'].$this->encryptdata['O'].$permissions.$this->encryptdata['fileid']);
				if ($this->encryptdata['mode'] > 0) {
					for ($i = 0; $i < 50; ++$i) {
						$tmp = Statics::_md5_16(substr($tmp, 0, $keybytelen));
					}
				}
			...
			// random 20-byte seed
			$seed = sha1(Statics::getRandomSeed(), true);
			$recipient_bytes = '';
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				// for each public certificate
				if (isset($pubkey['p'])) {
					$pkprotection = Statics::getUserPermissionCode($pubkey['p'], $this->encryptdata['mode']);
				} else {
					$pkprotection = $this->encryptdata['protection'];
				}
				// get default permissions (reverse byte order)
				$pkpermissions = Statics::getEncPermissionsString($pkprotection);
				// envelope data
				$envelope = $seed.$pkpermissions;
				// write the envelope data to a temporary file
				$tempkeyfile = Statics::getObjFilename('key', $this->file_id);
				$f = Statics::fopenLocal($tempkeyfile, 'wb');
				if (!$f) {
				...
				fclose($f);
				$tempencfile = Statics::getObjFilename('enc', $this->file_id);
	...
	public function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		$this->encryptdata['protection'] = Statics::getUserPermissionCode($permissions, $mode);
		...
		if ($owner_pass === null) {
			$owner_pass = md5(Statics::getRandomSeed());
		}
		...
		$this->encrypted = true;
		$this->encryptdata['fileid'] = Statics::convertHexStringToString($this->file_id);
	...
	public function Rect($x, $y, $w, $h, $style='', $border_style=array(), $fill_color=array()) {
		...
		if (!empty($style)) {
			$op = Statics::getPathPaintOperator($style);
	...
	public function Ellipse($x0, $y0, $rx, $ry='', $angle=0, $astart=0, $afinish=360, $style='', $line_style=array(), $fill_color=array(), $nc=2) {
		...
		if (Statics::empty_string($ry) OR ($ry == 0)) {
			$ry = $rx;
		}
		if (!(false === strpos($style, 'F')) AND isset($fill_color)) {
			$this->SetFillColorArray($fill_color);
	...
	public function setDestination($name, $y=-1, $page='', $x=-1) {
		...
		$name = Statics::encodeNameObject($name);
		if (Statics::empty_string($name)) {
			return false;
		}
		if ($y == -1) {
			$y = $this->GetY();
		...
		foreach ($this->outlines as $i => $o) {
			...
			if (isset($o['u']) AND !empty($o['u'])) {
				...
				if (is_string($o['u'])) {
					if ($o['u'][0] == '#') {
						// internal destination
						$out .= ' /Dest /'.Statics::encodeNameObject(substr($o['u'], 1));
					} elseif ($o['u'][0] == '%') {
						// embedded PDF file
	...
	public function TextField($name, $w, $h, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
		...
		// get annotation data
		$popt = Statics::getAnnotOptFromJSProp($prop, $this->spot_colors, $this->rtl);
	...
	public function RadioButton($name, $w, $prop=array(), $opt=array(), $onvalue='On', $checked=false, $x='', $y='', $js=false) {
		...
		if (Statics::empty_string($onvalue)) {
			$onvalue = 'On';
		}
		if ($checked) {
			$defval = $onvalue;
	...
	protected function _putsignature() {
		...
		if (empty($this->signature_data['approval']) OR ($this->signature_data['approval'] != 'A')) {
			...
			if ($this->signature_data['cert_type'] > 0) {
				...
				$out .= ' /Type /TransformParams';
				$out .= ' /V /2.2';
				if (!Statics::empty_string($this->ur['document'])) {
					$out .= ' /Document['.$this->ur['document'].']';
				}
				if (!Statics::empty_string($this->ur['form'])) {
					$out .= ' /Form['.$this->ur['form'].']';
				}
				if (!Statics::empty_string($this->ur['signature'])) {
					$out .= ' /Signature['.$this->ur['signature'].']';
				}
				if (!Statics::empty_string($this->ur['annots'])) {
					$out .= ' /Annots['.$this->ur['annots'].']';
				}
				if (!Statics::empty_string($this->ur['ef'])) {
					$out .= ' /EF['.$this->ur['ef'].']';
				}
				if (!Statics::empty_string($this->ur['formex'])) {
		...
		if (isset($this->signature_data['info']['Name']) AND !Statics::empty_string($this->signature_data['info']['Name'])) {
			$out .= ' /Name '.$this->_textstring($this->signature_data['info']['Name'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Location']) AND !Statics::empty_string($this->signature_data['info']['Location'])) {
			$out .= ' /Location '.$this->_textstring($this->signature_data['info']['Location'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Reason']) AND !Statics::empty_string($this->signature_data['info']['Reason'])) {
			$out .= ' /Reason '.$this->_textstring($this->signature_data['info']['Reason'], $sigobjid);
		}
		if (isset($this->signature_data['info']['ContactInfo']) AND !Statics::empty_string($this->signature_data['info']['ContactInfo'])) {
	...
	public function getAliasRightShift() {
		...
		$ref = '{'.Statics::$alias_right_shift.'}{'.Statics::$alias_tot_pages.'}{'.Statics::$alias_num_page.'}';
		...
		$alias = Statics::$alias_right_shift.$sdiff.'}';
		if ($this->isUnicodeFont()) {
			$alias = '{'.$alias;
		}
		return $alias;
	...
	public function getAliasNbPages() {
		if ($this->isUnicodeFont()) {
			return '{'.Statics::$alias_tot_pages.'}';
		}
		return Statics::$alias_tot_pages;
	}
	...
	public function getPageGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.Statics::$alias_group_tot_pages.'}';
		}
		return Statics::$alias_group_tot_pages;
	}
	...
	public function getPageNumGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.Statics::$alias_group_num_page.'}';
		}
		return Statics::$alias_group_num_page;
	}
	...
	public function getGroupPageNoFormatted() {
		return Statics::formatPageNumber($this->getGroupPageNo());
	}
	...
	public function PageNoFormatted() {
		return Statics::formatPageNumber($this->PageNo());
	}
	...
	public function setOverprint($stroking=true, $nonstroking='', $mode=0) {
		...
		if (Statics::empty_string($nonstroking)) {
	...
	public function ImageEps($file, $x='', $y='', $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0, $fitonpage=false, $fixoutvals=false) {
		...
		if ($file[0] === '@') { // image from string
			...
			$data = Statics::fileGetContents($file);
		}
		if ($data === FALSE) {
	...
	public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style='', $align='') {
		if (Statics::empty_string(trim($code))) {
			return;
		}
		// save current graphic settings
		$gvars = $this->getGraphicVars();
		// create new barcode object
		$barcodeobj = new Barcode($code, $type);
		...
		// print text
		if ($style['text']) {
			if (isset($style['label']) AND !Statics::empty_string($style['label'])) {
				$label = $style['label'];
			} else {
				$label = $code;
			}
	...
	public function fixHTMLCode($html, $default_css='', $tagvs='', $tidy_options='') {
		return Statics::fixHTMLCode($html, $default_css, $tagvs, $tidy_options, $this->tagvspaces);
	}
				...
				if (preg_match('/type[\s]*=[\s]*"text\/css"/', $link, $type)) {
					...
					if (empty($type) OR (isset($type[1]) AND (($type[1] == 'all') OR ($type[1] == 'print')))) {
						...
						if (preg_match('/href[\s]*=[\s]*"([^"]*)"/', $link, $type) > 0) {
							// read CSS data file
							$cssdata = Statics::fileGetContents(trim($type[1]));
							if (($cssdata !== FALSE) AND (strlen($cssdata) > 0)) {
								$css = array_merge($css, Statics::extractCSSproperties($cssdata));
							}
						}
					}
				}
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (($dom[$key]['value'] == 'tr') AND ($dom[($dom[$key]['parent'])]['thead'] === true)) {
						if (Statics::empty_string($dom[($dom[($dom[$key]['parent'])]['parent'])]['thead'])) {
					...
					if (($dom[$key]['value'] == 'table') AND (!Statics::empty_string($dom[($dom[$key]['parent'])]['thead']))) {
					...
					if (!empty($css)) {
						...
						list($dom[$key]['csssel'], $dom[$key]['cssdata']) = Statics::getCSSdataArray($dom, $key, $css);
						$dom[$key]['attribute']['style'] = Statics::getTagStyleFromCSSarray($dom[$key]['cssdata']);
					}
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						// font color
						if (isset($dom[$key]['style']['color']) AND (!Statics::empty_string($dom[$key]['style']['color']))) {
						...
						// background color
						if (isset($dom[$key]['style']['background-color']) AND (!Statics::empty_string($dom[$key]['style']['background-color']))) {
					...
					if ((($dom[$key]['value'] == 'ul') OR ($dom[$key]['value'] == 'ol') OR ($dom[$key]['value'] == 'dl'))
						AND (!isset($dom[$key]['align']) OR Statics::empty_string($dom[$key]['align']) OR ($dom[$key]['align'] != 'J'))) {
						if ($this->rtl) {
							$dom[$key]['align'] = 'R';
						} else {
							$dom[$key]['align'] = 'L';
					...
					if (isset($dom[$key]['attribute']['color']) AND (!Statics::empty_string($dom[$key]['attribute']['color']))) {
					...
					if (isset($dom[$key]['attribute']['bgcolor']) AND (!Statics::empty_string($dom[$key]['attribute']['bgcolor']))) {
						$dom[$key]['bgcolor'] = Colors::convertHTMLColorToDec($dom[$key]['attribute']['bgcolor'], $this->spot_colors);
					}
					// set stroke color attribute
					if (isset($dom[$key]['attribute']['strokecolor']) AND (!Statics::empty_string($dom[$key]['attribute']['strokecolor']))) {
					...
					// check for text alignment
					if (isset($dom[$key]['attribute']['align']) AND (!Statics::empty_string($dom[$key]['attribute']['align'])) AND ($dom[$key]['value'] !== 'img')) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
				if (isset($dom[$key]['parent']) AND isset($dom[$dom[$key]['parent']]['thead']) AND !Statics::empty_string($dom[$dom[$key]['parent']]['thead'])) {
					$this->inthead = true;
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				if (Statics::empty_string($lalign)) {
					$lalign = $align;
				}
			}
			// align lines
				...
				if (isset($startlinex)) {
					...
					if ((isset($plalign) AND ((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						if ($this->isRTLTextDir()) { // RTL
							...
							$pos1 = Statics::revstrpos($pmid, '[(');
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(Statics::revstrpos($pmid, '[('.chr(0).chr(32)));
									$spacelen = 2;
								} else {
									$pos2 = intval(Statics::revstrpos($pmid, '[('.chr(32)));
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(Statics::revstrpos($pmid, chr(0).chr(32).')]')) + 2;
									$spacelen = 2;
								} else {
									$pos2 = intval(Statics::revstrpos($pmid, chr(32).')]')) + 1;
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
								...
								while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
									...
									switch ($strpiece[2][0]) {
										...
										case 're': {
											// justify block
											if (!Statics::empty_string($this->lispacer)) {
												$this->lispacer = '';
												continue;
											}
			...
			if ($dom[$key]['tag']) {
				...
				// print list-item
				if (!Statics::empty_string($this->lispacer) AND ($this->lispacer != '^')) {
				...
				if (!isset($dom[$key]['trimmed_space']) OR !$dom[$key]['trimmed_space']) {
					...
					if (!empty($this->HREF) AND (isset($this->HREF['url']))) {
						...
						if (($strlinelen < $cwa) AND (isset($dom[($key + 1)])) AND ($dom[($key + 1)]['tag']) AND (!$dom[($key + 1)]['block'])) {
							...
							while ($write_block AND isset($dom[$nkey])) {
								if ($dom[$nkey]['tag']) {
									...
									$nextstr = Statics::pregSplit('/'.$this->re_space['p'].'+/', $this->re_space['m'], $dom[$nkey]['value']);
						...
						if (($wadj > 0) AND (($strlinelen + $wadj) >= $cwa)) {
							$wadj = 0;
							$nextstr = Statics::pregSplit('/'.$this->re_space['p'].'/', $this->re_space['m'], $dom[$key]['value']);
		...
		switch($tag['value']) {
			case 'table': {
				...
				if (!isset($dom[$key]['attribute']['nested']) OR ($dom[$key]['attribute']['nested'] != 'true')) {
					$this->htmlvspace = 0;
					// set table header
					if (!Statics::empty_string($dom[$key]['thead'])) {
						// set table header
			...
			case 'img': {
				if (!empty($tag['attribute']['src'])) {
					if ($tag['attribute']['src'][0] === '@') {
						...
						// get image type
						$type = Images::getImageFileType($tag['attribute']['src']);
					}
					if (!isset($tag['width'])) {
						$tag['width'] = 0;
					}
					...
					$xpos = $this->x;
					$imglink = '';
					if (isset($this->HREF['url']) AND !Statics::empty_string($this->HREF['url'])) {
			...
			case 'li': {
				...
				if ($this->listordered[$this->listnum]) {
					// ordered item
					if (isset($parent['attribute']['type']) AND !Statics::empty_string($parent['attribute']['type'])) {
						$this->lispacer = $parent['attribute']['type'];
					} elseif (isset($parent['listtype']) AND !Statics::empty_string($parent['listtype'])) {
						$this->lispacer = $parent['listtype'];
					} elseif (isset($this->lisymbol) AND !Statics::empty_string($this->lisymbol)) {
			...
			case 'input': {
				if (isset($tag['attribute']['name']) AND !Statics::empty_string($tag['attribute']['name'])) {
				...
				$prop = array();
				$opt = array();
				if (isset($tag['attribute']['readonly']) AND !Statics::empty_string($tag['attribute']['readonly'])) {
					$prop['readonly'] = true;
				}
				if (isset($tag['attribute']['value']) AND !Statics::empty_string($tag['attribute']['value'])) {
					$value = $tag['attribute']['value'];
				}
				if (isset($tag['attribute']['maxlength']) AND !Statics::empty_string($tag['attribute']['maxlength'])) {
					$opt['maxlen'] = intval($tag['attribute']['maxlength']);
				}
				$h = $this->getCellHeight($this->FontSize);
				if (isset($tag['attribute']['size']) AND !Statics::empty_string($tag['attribute']['size'])) {
				...
				switch ($tag['attribute']['type']) {
					...
					case 'image': {
						...
						if (isset($tag['attribute']['src']) AND !Statics::empty_string($tag['attribute']['src'])) {
			...
			case 'textarea': {
				...
				if (isset($tag['attribute']['cols']) AND !Statics::empty_string($tag['attribute']['cols'])) {
					$w = intval($tag['attribute']['cols']) * $this->GetStringWidth(chr(32)) * 2;
				} else {
					$w = 40;
				}
				if (isset($tag['attribute']['rows']) AND !Statics::empty_string($tag['attribute']['rows'])) {
			...
			case 'select': {
				...
				$w = 0;
				if (isset($tag['attribute']['opt']) AND !Statics::empty_string($tag['attribute']['opt'])) {
	...
	protected function drawHTMLTagBorder($tag, $xmax) {
		...
		if (isset($tag['border']) AND !empty($tag['border'])) {
			...
			if (!Statics::empty_string($this->thead) AND (!$this->inthead)) {
				...
				$border = Statics::getBorderMode($border, $position='middle', $this->opencell);
			}
		}
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		switch ($listtype) {
			...
			case 'i':
			case 'lower-roman': {
				$textitem = strtolower(Statics::intToRoman($this->listcount[$this->listnum]));
				break;
			}
			case 'I':
			case 'upper-roman': {
				$textitem = Statics::intToRoman($this->listcount[$this->listnum]);
				break;
			}
			case 'a':
			case 'lower-alpha':
		...
		if (!Statics::empty_string($textitem)) {
	...
	public function addTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		$page_fill_end = false;
		$current_column = $this->current_column;
		if (Statics::empty_string($numbersfont)) {
			$numbersfont = $this->default_monospaced_font;
		}
		if (Statics::empty_string($filler)) {
			$filler = ' ';
		}
		if (Statics::empty_string($page)) {
			$gap = ' ';
		} else {
			$gap = '';
			if ($page < 1) {
		...
		if (!Statics::empty_string($page)) {
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					$na = Statics::formatTOCPageNumber(($this->starting_page_number + $np - 1));
	...
	public function startTransaction() {
		...
		$this->objcopy = Statics::objclone($this);
	}
	...
	*/
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		if (!is_array($patterns)) {
			$patterns = Statics::getHyphenPatternsFromTEX($patterns);
		}
	...
	public function printTemplate($id, $x='', $y='', $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
		...
		if (!empty($this->xobjects[$id]['annotations'])) {
			foreach ($this->xobjects[$id]['annotations'] as $annot) {
				...
				$coordlt = Statics::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, ($annot['x'] * $this->k), (-$annot['y'] * $this->k)));
				$ax = ($coordlt[4] / $this->k);
				$ay = ($this->h - $h - ($coordlt[5] / $this->k));
				$coordrb = Statics::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, (($annot['x'] + $annot['w']) * $this->k), ((-$annot['y'] - $annot['h']) * $this->k)));
	...
	public function ImageSVG($file, $x='', $y='', $w=0, $h=0, $link='', $align='', $palign='', $border=0, $fitonpage=false) {
		...
		if ($file[0] === '@') { // image from string
			...
			$svgdata = Statics::fileGetContents($file);
		}
		if ($svgdata === FALSE) {
					...
					foreach ($params as $ck => $cp) {
						...
						if ((($ck + 1) % 7) == 0) {
							...
							if ((abs($x0 - $x) < $minlen) AND (abs($y0 - $y) < $minlen)) {
								...
								// get angles
								$angs = Statics::getVectorsAngle(1, 0, (($xa - $cax) / $rx), (($cay - $ya) / $ry));
								$dang = Statics::getVectorsAngle((($xa - $cax) / $rx), (($ya - $cay) / $ry), ((-$xa - $cax) / $rx), ((-$ya - $cay) / $ry));
								if (($fs == 0) AND ($dang > 0)) {
									$dang -= (2 * M_PI);
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if (isset($attribs['style']) AND !Statics::empty_string($attribs['style']) AND ($attribs['style'][0] != ';')) {
		...
		foreach ($prev_svgstyle as $key => $val) {
			...
			if (isset($attribs[$key]) AND !Statics::empty_string($attribs[$key])) {
			...
			} elseif (isset($attribs['style']) AND !Statics::empty_string($attribs['style'])) {
				// CSS style syntax
				$attrval = array();
		...
		if (isset($attribs['transform']) AND !empty($attribs['transform'])) {
			$tm = Statics::getTransformationMatrixProduct($tm, Statics::getSVGTransformMatrix($attribs['transform']));
		}
		...
		switch($name) {
			...
			case 'svg': {
				...
				$tm = Statics::getTransformationMatrixProduct($tm, array( 1, 0, 0, 1, $svgX, $svgY));
				...
				if (isset($attribs['viewBox'])) {
					...
					if (sizeof($tmp) == 4) {
						...
						$tm = Statics::getTransformationMatrixProduct($tm, $newtm);
						$this->SVGTransform($tm);
					}
				}
			...
			case 'g': {
				...
				$tm = Statics::getTransformationMatrixProduct($tm, array($w, 0, 0, $h, $x, $y));
				$this->SVGTransform($tm);
				$this->setSVGStyles($svgstyle, $prev_svgstyle);
				break;
			}
			case 'linearGradient': {
				...
				if (isset($attribs['gradientTransform'])) {
					$this->svggradients[$this->svggradientid]['gradientTransform'] = Statics::getSVGTransformMatrix($attribs['gradientTransform']);
				}
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						// fix image path
						if (!Statics::empty_string($this->svgdir) AND (($img[0] == '.') OR (basename($img) == $img))) {

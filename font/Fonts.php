...
class Fonts {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if (empty($fonttype)) {
			if (Statics::_getULONG($font, 0) == 0x10000) {
		...
		if ($fmetric['type'] == 'Type1') {
			...
			$fmetric['file'] .= '.z';
			$fp = Statics::fopenLocal($outpath.$fmetric['file'], 'wb');
			...
			if (Statics::_getULONG($font, $offset) != 0x10000) {
			...
			$numTables = Statics::_getUSHORT($font, $offset);
			$offset += 2;
			...
			for ($i = 0; $i < $numTables; ++$i) {
				...
				$offset += 4;
				$table[$tag] = array();
				$table[$tag]['checkSum'] = Statics::_getULONG($font, $offset);
				$offset += 4;
				$table[$tag]['offset'] = Statics::_getULONG($font, $offset);
				$offset += 4;
				$table[$tag]['length'] = Statics::_getULONG($font, $offset);
				$offset += 4;
			}
			// check magicNumber
			$offset = $table['head']['offset'] + 12;
			if (Statics::_getULONG($font, $offset) != 0x5F0F3CF5) {
			...
			$offset += 2; // skip flags
			// get FUnits
			$fmetric['unitsPerEm'] = Statics::_getUSHORT($font, $offset);
			$offset += 2;
			// units ratio constant
			$urk = (1000 / $fmetric['unitsPerEm']);
			$offset += 16; // skip created, modified
			$xMin = round(Statics::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$yMin = round(Statics::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$xMax = round(Statics::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$yMax = round(Statics::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$fmetric['bbox'] = ''.$xMin.' '.$yMin.' '.$xMax.' '.$yMax.'';
			$macStyle = Statics::_getUSHORT($font, $offset);
			$offset += 2;
			// PDF font flags
			$fmetric['Flags'] = $flags;
			if (($macStyle & 2) == 2) {
			...
			$short_offset = (Statics::_getSHORT($font, $offset) == 0);
			$offset += 2;
			...
			if ($short_offset) {
				...
				for ($i = 0; $i < $tot_num_glyphs; ++$i) {
					$indexToLoc[$i] = Statics::_getUSHORT($font, $offset) * 2;
				...
				for ($i = 0; $i < $tot_num_glyphs; ++$i) {
					$indexToLoc[$i] = Statics::_getULONG($font, $offset);
					if (isset($indexToLoc[($i - 1)]) && ($indexToLoc[$i] == $indexToLoc[($i - 1)])) {
						// the last glyph didn't have an outline
						unset($indexToLoc[($i - 1)]);
					}
			...
			$numEncodingTables = Statics::_getUSHORT($font, $offset);
			$offset += 2;
			$encodingTables = array();
			for ($i = 0; $i < $numEncodingTables; ++$i) {
				$encodingTables[$i]['platformID'] = Statics::_getUSHORT($font, $offset);
				$offset += 2;
				$encodingTables[$i]['encodingID'] = Statics::_getUSHORT($font, $offset);
				$offset += 2;
				$encodingTables[$i]['offset'] = Statics::_getULONG($font, $offset);
				$offset += 4;
			}
			// ---------- get os/2 metrics ----------
			...
			// xAvgCharWidth
			$fmetric['AvgWidth'] = round(Statics::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			// usWeightClass
			$usWeightClass = round(Statics::_getUFWORD($font, $offset) * $urk);
			...
			$fsType = Statics::_getSHORT($font, $offset);
			$offset += 2;
			if ($fsType == 2) {
			...
			$numNameRecords = Statics::_getUSHORT($font, $offset);
			$offset += 2;
			// Offset to start of string storage (from start of table).
			$stringStorageOffset = Statics::_getUSHORT($font, $offset);
			$offset += 2;
			for ($i = 0; $i < $numNameRecords; ++$i) {
				$offset += 6; // skip Platform ID, Platform-specific encoding ID, Language ID.
				// Name ID.
				$nameID = Statics::_getUSHORT($font, $offset);
				$offset += 2;
				if ($nameID == 6) {
					// String length (in bytes).
					$stringLength = Statics::_getUSHORT($font, $offset);
					$offset += 2;
					// String offset from start of storage area (in bytes).
					$stringOffset = Statics::_getUSHORT($font, $offset);
					$offset += 2;
			...
			$fmetric['italicAngle'] = Statics::_getFIXED($font, $offset);
			$offset += 4;
			$fmetric['underlinePosition'] = round(Statics::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$fmetric['underlineThickness'] = round(Statics::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$isFixedPitch = (Statics::_getULONG($font, $offset) == 0) ? false : true;
			$offset += 2;
			if ($isFixedPitch) {
				$fmetric['Flags'] |= 1;
			}
			...
			// Ascender
			$fmetric['Ascent'] = round(Statics::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			// Descender
			$fmetric['Descent'] = round(Statics::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			// LineGap
			$fmetric['Leading'] = round(Statics::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			// advanceWidthMax
			$fmetric['MaxWidth'] = round(Statics::_getUFWORD($font, $offset) * $urk);
			$offset += 2;
			$offset += 22; // skip some values
			// get the number of hMetric entries in hmtx table
			$numberOfHMetrics = Statics::_getUSHORT($font, $offset);
			// ---------- get maxp data ----------
			$offset = $table['maxp']['offset'];
			$offset += 4; // skip Table version number
			// get the the number of glyphs in the font.
			$numGlyphs = Statics::_getUSHORT($font, $offset);
			// ---------- get CIDToGIDMap ----------
			$ctg = array();
					...
					switch ($format) {
						...
						case 2: { // Format 2: High-byte mapping through table
							...
							for ($i = 0; $i < 256; ++$i) {
								...
								$subHeaderKeys[$i] = (Statics::_getUSHORT($font, $offset) / 8);
								$offset += 2;
							...
							for ($k = 0; $k < $numSubHeaders; ++$k) {
								$subHeaders[$k]['firstCode'] = Statics::_getUSHORT($font, $offset);
								$offset += 2;
								$subHeaders[$k]['entryCount'] = Statics::_getUSHORT($font, $offset);
								$offset += 2;
								$subHeaders[$k]['idDelta'] = Statics::_getUSHORT($font, $offset);
								$offset += 2;
								$subHeaders[$k]['idRangeOffset'] = Statics::_getUSHORT($font, $offset);
								$offset += 2;
							...
							for ($k = 0; $k < $numGlyphIndexArray; ++$k) {
								$glyphIndexArray[$k] = Statics::_getUSHORT($font, $offset);
								$offset += 2;
							}
							for ($i = 0; $i < 256; ++$i) {
								$k = $subHeaderKeys[$i];
						...
						case 6: { // Format 6: Trimmed table mapping
							$offset += 4; // skip length and version/language
							$firstCode = Statics::_getUSHORT($font, $offset);
							$offset += 2;
							$entryCount = Statics::_getUSHORT($font, $offset);
							$offset += 2;
							for ($k = 0; $k < $entryCount; ++$k) {
								$c = ($k + $firstCode);
								$g = Statics::_getUSHORT($font, $offset);
								$offset += 2;
								$ctg[$c] = $g;
							}
							break;
						...
						case 8: { // Format 8: Mixed 16-bit and 32-bit coverage
							...
							for ($k = 0; $k < 8192; ++$k) {
								$is32[$k] = Statics::_getBYTE($font, $offset);
								++$offset;
							}
							$nGroups = Statics::_getULONG($font, $offset);
							$offset += 4;
							for ($i = 0; $i < $nGroups; ++$i) {
								$startCharCode = Statics::_getULONG($font, $offset);
								$offset += 4;
								$endCharCode = Statics::_getULONG($font, $offset);
								$offset += 4;
								$startGlyphID = Statics::_getULONG($font, $offset);
								$offset += 4;
						...
						case 10: { // Format 10: Trimmed array
							...
							$startCharCode = Statics::_getULONG($font, $offset);
							$offset += 4;
							$numChars = Statics::_getULONG($font, $offset);
							$offset += 4;
						...
						case 12: { // Format 12: Segmented coverage
							...
							for ($k = 0; $k < $nGroups; ++$k) {
								...
								$endCharCode = Statics::_getULONG($font, $offset);
								$offset += 4;
								$startGlyphCode = Statics::_getULONG($font, $offset);
								$offset += 4;
			...
			$yMin = Statics::_getFWORD($font, $offset);
			$offset += 4;
			$yMax = Statics::_getFWORD($font, $offset);
			$offset += 2;
			...
			for ($i = 0 ; $i < $numberOfHMetrics; ++$i) {
				$cw[$i] = round(Statics::_getUFWORD($font, $offset) * $urk);
				$offset += 4; // skip lsb
			}
			...
			for ($cid = 0; $cid <= 65535; ++$cid) {
				if (isset($ctg[$cid])) {
					...
					if ($addcbbox AND isset($indexToLoc[$ctg[$cid]])) {
						...
						$xMin = round(Statics::_getFWORD($font, $offset + 2) * $urk);
						$yMin = round(Statics::_getFWORD($font, $offset + 4) * $urk);
						$xMax = round(Statics::_getFWORD($font, $offset + 6) * $urk);
						$yMax = round(Statics::_getFWORD($font, $offset + 8) * $urk);
		...
		if ($fmetric['type'] == 'Type1') {
			...
			if ($fmetric['type'] == 'cidfont0') {
				...
				$fp = Statics::fopenLocal($outpath.$fmetric['ctg'], 'wb');
		...
		$pfile .= '// --- EOF ---'."\n";
		// store file
		$fp = Statics::fopenLocal($outpath.$font_name.'.php', 'w');
		fwrite($fp, $pfile);
		fclose($fp);
		...
		while (!empty($new_sga)) {
			...
			foreach ($sga as $key => $val) {
				if (isset($indexToLoc[$key])) {
					...
					$numberOfContours = Statics::_getSHORT($font, $offset);
					$offset += 2;
					if ($numberOfContours < 0) { // composite glyph
						$offset += 8; // skip xMin, yMin, xMax, yMax
						do {
							$flags = Statics::_getUSHORT($font, $offset);
							$offset += 2;
							$glyphIndex = Statics::_getUSHORT($font, $offset);
							$offset += 2;
	...
	public static function _getfontpath() {
		if (!defined('K_PATH_FONTS') AND is_dir($fdir = realpath(dirname(__FILE__).'/../common/fonts'))) {
			if (substr($fdir, -1) != '/') {
				$fdir .= '/';
			}
...
// ====================================================================================================================
	...
	public static function UTF8ArrayToUniArray($ta, $isunicode=true) {
		if ($isunicode) {
			return array_map(array('\tcpdf\font\Fonts', 'unichrUnicode'), $ta);
		}
		return array_map(array('\tcpdf\font\Fonts', 'unichrASCII'), $ta);
	}
	...
	public static function UTF8StringToArray($str, $isunicode=true, &$currentfont) {
		if ($isunicode) {
			...
			$chars = Statics::pregSplit('//','u', $str, -1, PREG_SPLIT_NO_EMPTY);
			$carr = array_map(array('\tcpdf\font\Fonts', 'uniord'), $chars);
		} else {
			$chars = str_split($str);
